...
void ConfFile::setComment(string com)
{
	comment = com;
}
...
void ConfFile::save()
{
	...
	//写入注释
	wStream << "# Generated by ConfWritter" << endl;

	//写入自定义注释
	if (comment!="")
	{
		wStream << "# "<< comment << endl;
	}
	...
	if (getSectionsSzie() > 0) {
		...
		for (i = 0; i < getSectionsSzie(); i++) {
			...
			if (mSection.getItemsSize() > 0) {
				...
				for (j = 0; j < mSection.getItemsSize(); j++) {
